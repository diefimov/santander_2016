...
pop_split=read.csv('data/output/features/population_split.csv')
full=left_join(full,pop_split,by=c('ID'='ID'))
...
full$xgb_raddar1=Reduce('+',cv.set)/20

write.csv(select(filter(full,!is.na(TARGET)),ID,xgb_raddar1),'data/output/temp/xgb_raddar_1_train.csv',row.names=FALSE)
write.csv(select(filter(full,is.na(TARGET)),ID,xgb_raddar1),'data/output/temp/xgb_raddar_1_test.csv',row.names=FALSE)
