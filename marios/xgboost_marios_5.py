...
def main():
        ...
        SEED=15
        outset="xgboost_marios_5" # predic of all files
        ...
        model=xg.XGBoostClassifier(num_round=1000 ,nthread=62,  eta=0.009, gamma=0.1,max_depth=5, min_child_weight=1, subsample=0.9, 
                                   colsample_bytree=0.25,objective='binary:logistic',num_parallel_tree=1, seed=1)               
        ...
        for kfolder in kfolders:
            ...
            if number_of_folds>0:
                for train_index, test_index in kfolder:
                    ...
                    woe1_train, woe1_cv= convert_to_woe(np.round(X_train,2),y_train, np.round(X_cv,2), seed=1, cvals=5, roundings=2, columns=None)

                    X_train=np.column_stack((X_train,woe_train))
                    X_cv=np.column_stack((X_cv,woe_cv))          

                    X_train=np.column_stack((X_train,woe1_train))
                    X_cv=np.column_stack((X_cv,woe1_cv))       
        ...
        woe1_train, woe1_cv= convert_to_woe(np.round(X,2),y, np.round(X_test,2), seed=1, cvals=5, roundings=2, columns=None)               
        X=np.column_stack((X,woe_train))
        X_test=np.column_stack((X_test,woe_cv))  

         
        X=np.column_stack((X,woe1_train))
        X_test=np.column_stack((X_test,woe1_cv))   
