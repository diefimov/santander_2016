...
def main():
        ...
        SEED=15
        outset="xgboost_marios_2" # predic of all files
        ...
        model=xg.XGBoostClassifier(num_round=1000 ,nthread=30,  eta=0.009, gamma=0.1,max_depth=5, min_child_weight=1, subsample=0.9, 
        ...
        for kfolder in kfolders:
            ...
            if number_of_folds>0:
                for train_index, test_index in kfolder:
                    ...
                    #create weights of evidence
                    woe_train, woe_cv= convert_to_woe(np.round(X_train,2),y_train, np.round(X_cv,2), seed=1, cvals=5, roundings=2, columns=None)
