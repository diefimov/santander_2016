...
def build_model(input_dim, output_dim):
    ...
    models.add(Dense(100, input_dim=input_dim, init='glorot_uniform', W_regularizer=l2(0.00001)))
    models.add(PReLU())
    models.add(BatchNormalization())
    models.add(Dropout(0.6))
    models.add(Dense(100, init='glorot_uniform', W_regularizer=l2(0.00001)))
    models.add(PReLU())
    models.add(BatchNormalization())
    models.add(Dropout(0.60))
    models.add(Dense(20, init='glorot_uniform', W_regularizer=l2(0.00001)))
    models.add(PReLU())
    models.add(BatchNormalization())
    models.add(Dropout(0.20))    
    models.add(Dense(output_dim, init='glorot_uniform'))
...
def main():
        ...
        SEED=15
        outset="nn_marios_1_3" # predic of all files
        ...
        for kfolder in kfolders:
            ...
            if number_of_folds>0:
                for train_index, test_index in kfolder:
                    ...
                    #id_train, id_cv = np.array(idstrain)[train_index], np.array(idstrain)[test_index]                
                    ...
                    """
                    for t in range(0, len(id_cv)):
                        if id_cv[t] in dictionary:
                          preds[t]=0.0 
                    """                     
